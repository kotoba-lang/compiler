...
(defn- invoke-function [function values functions fuel call-stack cap-call]
  (when-not function
  ...
  (eval-expr (:body function) (zipmap (:params function) values) functions
             fuel (conj call-stack (:name function)) cap-call))

(defn eval-expr [form env functions fuel call-stack cap-call]
  (cond
      ...
      (cond
        ...
        (let [[bindings body] args
              env' (reduce (fn [e [name value]]
                             (assoc e name (eval-expr value e functions fuel call-stack cap-call)))
                           env (partition 2 bindings))]
          (eval-expr body env' functions fuel call-stack cap-call))

        (= op 'if)
        (let [[test then else] args]
          (eval-expr (if (zero? (eval-expr test env functions fuel call-stack cap-call))
                       else then)
                     env functions fuel call-stack cap-call))

        (= op 'cap-call)
        (let [[cap-id value] args]
          (when-not cap-call
            (trap! :capability-denied {:capability cap-id}))
          (long (cap-call cap-id (eval-expr value env functions fuel call-stack cap-call))))

        (contains? '#{+ - * quot = < > <= >=} op)
        (let [xs (mapv #(eval-expr % env functions fuel call-stack cap-call) args)]
          (case op
            + (reduce i64-add xs)
        ...
        :else
        (let [values (mapv #(eval-expr % env functions fuel call-stack cap-call) args)]
          (invoke-function (get functions op) values functions fuel call-stack cap-call))))))
...
(defn execute
  ...
  ([kir function-name args {:keys [fuel cap-call] :or {fuel default-fuel}}]
