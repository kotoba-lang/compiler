...
attested_run_check() {
  ...
  grep -q ':format :kotoba.native-runtime/v5' "$TMP/$ISA-run-result.edn"
  grep -Eq ':loader-binary-sha256 "[0-9a-f]{64}"' "$TMP/$ISA-run-result.edn"
  grep -Eq ':assembler-binary-sha256 "[0-9a-f]{64}"' "$TMP/$ISA-run-result.edn"
  grep -Eq ':linker-binary-sha256 "[0-9a-f]{64}"' "$TMP/$ISA-run-result.edn"
  grep -Eq ':compiler-resource-sha256 "[0-9a-f]{64}"' "$TMP/$ISA-run-result.edn"
  grep -Eq ':system-header-closure-sha256 "[0-9a-f]{64}"' "$TMP/$ISA-run-result.edn"
